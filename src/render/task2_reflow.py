...
def task2_blocks(spec: dict[str, Any], task_output: dict[str, Any], _: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    metric_blocks = [
        block(
            ...
            [
                f"В числах это даёт: при r = {r1['x_value']} получаем M_пр = {format_teacher_number(r1['metrics']['idle_machines_expected'])} и M_ож = {format_teacher_number(r1['metrics']['waiting_machines_expected'])}; при r = {r10['x_value']} — {format_teacher_number(r10['metrics']['idle_machines_expected'])} и {format_teacher_number(r10['metrics']['waiting_machines_expected'])}.",
        ...
        block(
            ...
            [
                ...
                f"Для ориентира: P_ож убывает с {format_teacher_number(r1['metrics']['waiting_probability'])} при r = {r1['x_value']} до {format_teacher_number(r10['metrics']['waiting_probability'])} при r = {r10['x_value']} и до {format_teacher_number(r20['metrics']['waiting_probability'])} при r = {r20['x_value']}.",
            ...
            [spec["figure_ids"][2]],
            ["График подтверждает, что после выхода из режима острой нехватки наладчиков эта вероятность сначала убывает быстрее всего, а затем продолжает снижаться по мере уменьшения очереди."],
        ),
        block(
            ...
            [
                f"По расчёту: при r = {r5['x_value']} имеем M_зан = {format_teacher_number(r5['metrics']['busy_repairers_expected'])} и K_загр = {format_teacher_number(r5['metrics']['repairers_utilization'])}; при r = {r10['x_value']} — {format_teacher_number(r10['metrics']['busy_repairers_expected'])} и {format_teacher_number(r10['metrics']['repairers_utilization'])}.",
