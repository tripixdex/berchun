...
def task2_blocks(spec: dict[str, Any], task_output: dict[str, Any], _: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    metric_blocks = [
        block(
            ...
            [
                f"В числах это даёт: при r = 1 получаем M_пр = {format_teacher_number(r1['metrics']['idle_machines_expected'])} и M_ож = {format_teacher_number(r1['metrics']['waiting_machines_expected'])}; при r = 10 — {format_teacher_number(r10['metrics']['idle_machines_expected'])} и {format_teacher_number(r10['metrics']['waiting_machines_expected'])}.",
        ...
        block(
            ...
            [
                ...
                f"Для ориентира: P_ож убывает с {format_teacher_number(r1['metrics']['waiting_probability'])} при r = 1 до {format_teacher_number(r10['metrics']['waiting_probability'])} при r = 10 и до {format_teacher_number(r20['metrics']['waiting_probability'])} при r = 20.",
                f"В характерной точке r = 10 вероятность того, что новый отказ будет ожидать обслуживания, равна {format_teacher_number(r10['metrics']['waiting_probability'])}, тогда как стационарная вероятность наличия очереди составляет {queue_state_share}.",
            ],
            [spec["figure_ids"][2]],
        ...
        block(
            ...
            [
                f"По расчёту: при r = 5 имеем M_зан = {format_teacher_number(r5['metrics']['busy_repairers_expected'])} и K_загр = {format_teacher_number(r5['metrics']['repairers_utilization'])}; при r = 10 — {format_teacher_number(r10['metrics']['busy_repairers_expected'])} и {format_teacher_number(r10['metrics']['repairers_utilization'])}.",
