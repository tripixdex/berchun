...
def task11_blocks(spec: dict[str, Any], task_output: dict[str, Any], derived: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    state_blocks = [
        block(
            ...
            ["Сначала по Tc и Ts находим интенсивности потока и обслуживания, а затем выражаем через них стационарные вероятности состояний S_k, где k равно числу занятых операторов."],
            spec["state_formulas"],
            [
                f"Для текущего варианта offered load равен {load}, поэтому даже в системе без очереди крайнее состояние S_n заметно влияет на итоговые метрики.",
    ...
    metric_blocks = [
        block(
            ...
            ["Сначала по найденным p_k суммируем среднее число занятых операторов, а затем нормируем его по n и получаем коэффициент загрузки."],
            [spec["metric_formulas"][1], spec["metric_formulas"][2]],
            [f"Числовой checkpoint: при n = 5 имеем M_зан = {format_teacher_number(n5['busy_operators_expected'])} и K_загр = {format_teacher_number(n5['operators_utilization'])}; при n = 8 — {format_teacher_number(n8['busy_operators_expected'])} и {format_teacher_number(n8['operators_utilization'])}."],
            [spec["figure_ids"][0]],
        ...
        block(
            "Вероятность отказа.",
            ["Затем рассматриваем крайнее состояние S_n: именно оно соответствует ситуации, когда новый звонок приходит в момент полной занятости всех операторов."],
            [spec["metric_formulas"][0]],
            [f"Числовой checkpoint: при n = 7 вероятность отказа ещё равна {format_teacher_number(n7['refusal_probability'])}, а при n = 8 уже снижается до {format_teacher_number(n8['refusal_probability'])}."],
            [spec["figure_ids"][1]],
            [
...
def task12_blocks(spec: dict[str, Any], task_output: dict[str, Any], _: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    state_blocks = [
        block(
            ...
            ["После состояний обслуживания добавляются состояния ожидания, поэтому сначала выписываем offered load, коэффициент ρ_n и нормировочную формулу p_0."],
        ...
        block(
            ...
            ["Затем разделяем вероятности на две части: состояния k = 0, ..., n описывают обслуживание, а состояния n + r — очередь длины r."],
            spec["state_formulas"][2:],
            [
    ...
    metric_blocks = [
        block("Семейства по числу мест в очереди.", ["Сначала фиксируем число операторов n и смотрим, как изменение m перераспределяет потери и ожидание."]),
        block(
            "Вероятность отказа.",
            ["При фиксированном n отказ определяется последним состоянием цепи с заполненной очередью."],
            [spec["metric_formulas"][0]],
            [f"Числовой checkpoint: при n = 5 рост m от 1 до 5 снижает P_отк с {format_teacher_number(n5m1['refusal_probability'])} до {format_teacher_number(n5m5['refusal_probability'])}, а при m = 15 вероятность отказа становится {format_teacher_number(n5m15['refusal_probability'])}."],
            [spec["figure_ids"][0]],
        ...
        block("Семейства по числу операторов.", ["Во втором sweep, наоборот, фиксируем m и смотрим, как добавление операторов меняет те же метрики."]),
        block(
            "Вероятность отказа.",
            ...
            [],
            [f"Числовой checkpoint: при m = 5 увеличение числа операторов с n = 5 до n = 8 снижает P_отк с {format_teacher_number(m5n5['refusal_probability'])} до {format_teacher_number(m5n8['refusal_probability'])}; при n = 11 оно остаётся на уровне {format_teacher_number(m5n11['refusal_probability'])}."],
            [spec["figure_ids"][6]],
        ...
        block(
            "Занятые операторы и загрузка.",
            ["На том же sweep видно, что абсолютная занятость системы меняется мало, зато загрузка каждого отдельного оператора заметно падает."],
            [],
