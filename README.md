...
- если YAML не проходит валидацию, CLI объясняет, какое поле сломано, и сразу предлагает простой recovery: Enter повторяет проверку, `e` возвращает к выбору YAML, `s` создаёт starter YAML, `x` отменяет;
...
- `study_pack` + `bundle_dir` + `variant_aware` + selectable report/guide inner formats
- `study_pack` + `bundle_dir` + `general` + selectable report/guide inner formats
...
- `study_pack/general` теперь реально включает один или оба bundle-local report surface по выбору оператора, плюс `guide/methodical_guide__general.md` и один или оба bundle-local guide surface по выбору оператора, а также guide schemes, но без guide plots;
...
- при `study_pack` top-level `output_format` по-прежнему остаётся `bundle_dir`, но внутри комплекта оператор отдельно выбирает format policy для report и guide: `PDF`, `DOCX` или `PDF + DOCX`;
- `print_pack` по-прежнему не получает guide PDF/DOCX copies в текущем runtime slice;
