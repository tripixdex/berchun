...
- λ = 1/Tc = 0.037 1/с, μ = 1/Ts = 0.02 1/с.
...
При этих исходных данных приведённая нагрузка равна 1.815, поэтому даже в системе без очереди крайнее состояние S_n заметно влияет на итоговые метрики.
...
На опорных точках получаем: при n = 5 имеем M_зан = 1.766 и K_загр = 0.353; при n = 6 — 1.8 и 0.3.

На кривой видно, что после n = 5 среднее число занятых операторов растёт уже слабо: добавление новых операторов дальше создаёт прежде всего запас по отказам, а не существенный прирост фактической занятости.
...
Для сравнения: при n = 5 вероятность отказа равна 0.027, а при n = 6 — 0.008.

Кривая отказов показывает, что условие P_отк < 0.01 впервые выполняется при n = 6.

В этой точке вероятность отказа равна 0.008, поэтому значение n = 6 принимается как минимально достаточное для текущего варианта.

#### Короткий вывод

По графику отказов условие P_отк < 0.01 впервые выполняется при n = 6.
...
По расчёту: при n = 5 рост m от 1 до 5 снижает P_отк с 0.01 до < 0.001, а при m = 15 вероятность отказа становится < 0.001.
...
При том же n = 5 среднее число занятых операторов увеличивается лишь с 1.797 до 1.815, а коэффициент загрузки — с 0.359 до 0.363.
...
При n = 5 переход от m = 1 к m = 15 увеличивает вероятность существования очереди с 0.01 до 0.015, а среднюю длину очереди — с 0.01 до 0.024.

При этом занятость одного места изменяется с 0.01 до 0.002; по этому показателю важно смотреть на конкретные численные значения для выбранного режима нагрузки.
...
Для m = 5 среднее число занятых операторов меняется с 1.815 при n = 5 до 1.815 при n = 11, а коэффициент загрузки падает с 0.363 до 0.165.
...
При m = 5 вероятность существования очереди снижается с 0.015 при n = 5 до < 0.001 при n = 8 и до < 0.001 при n = 11.

Средняя длина очереди за тот же переход падает с 0.023 до < 0.001 и далее до < 0.001.
...
При данных параметрах a = 1.815, поэтому условие ρ_n < 1 впервые выполняется только при n = 2.

Поэтому режимы n = 1 остаются нестационарными и в графиках не получают выдуманных стационарных значений.
...
В первой стационарной точке n = 2 получаем P_ож = 0.863, поэтому уже первая стационарная точка соответствует высокой вероятности ожидания.
...
На первых стационарных точках M_зан остаётся равным 1.815, но коэффициент загрузки падает с 0.907 при n = 2 до 0.454 при n = 4 и 0.259 при n = 7.
...
По расчёту: при n = 2 имеем P_оч = 0.783 и L_оч = 8.461; уже при n = 4 эти значения снижаются до 0.06 и 0.109.

При n = 7 очередь практически исчезает: P_оч = < 0.001, L_оч = < 0.001.
...
Нестационарными остаются режимы n = 1; для них стационарные метрики в отчёте не подставляются.

Первое стационарное значение достигается при n = 2.
...
- λ = 0.037 1/с, μ = 0.02 1/с, ν = 1/Tw = 0.009 1/с.
...
Оставшийся вклад не превышает 1.726e-13 по вероятности и 9.542e-13 по среднему числу заявок в очереди, поэтому он пренебрежим в пределах учебной точности отчёта.
...
На характерных точках: при n = 1 получаем M_зан = 0.913 и K_загр = 0.913; при n = 5 — 1.807 и 0.361; при n = 8 — 1.815 и 0.227.
...
Для сравнения: при n = 1 вероятность существования очереди равна 0.754, а средняя длина очереди — 2.062.

При n = 5 эти значения снижаются до 0.013 и 0.018, а при n = 8 — до < 0.001 и < 0.001.
...
Оставшийся после усечения вклад не превышает 1.726e-13 по вероятности и 9.542e-13 по среднему числу заявок в очереди, поэтому их вклад пренебрежим в пределах учебной точности данного отчёта.

Например, при n = 1 вероятность существования очереди составляет 0.754, а средняя длина очереди равна 2.062; при n = 8 эти значения снижаются до < 0.001 и < 0.001.
...
- λ = 1/Tc = 0.009 1/мин, μ = 1/Ts = 0.029 1/мин.
...
На опорных точках получаем: при r = 1 имеем M_пр = 38.559 и M_ож = 37.559; при r = 10 — 11.077 и 2.091.
...
Для сравнения: P_ож убывает с 1 при r = 1 до 0.569 при r = 10 и до < 0.001 при r = 20.

В характерной точке r = 10 вероятность того, что новый отказ будет ожидать обслуживания, равна 0.569, тогда как стационарная вероятность наличия очереди составляет 0.511.
...
На характерных точках: при r = 5 имеем M_зан = 5 и K_загр = 1; при r = 10 — 8.986 и 0.899.

Даже при r = 33 в среднем заняты лишь 9.457 наладчика, поэтому коэффициент загрузки снижается до 0.287.
...
Для характерной точки r = 10 получаем P_ож = 0.569, тогда как стационарная вероятность наличия очереди равна 0.511; среднее число ожидающих станков при этом составляет 2.091.
