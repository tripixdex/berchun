...
- λ = 1/Tc = 0.059 1/с, μ = 1/Ts = 0.017 1/с.
...
При этих исходных данных приведённая нагрузка равна 3.412, поэтому даже в системе без очереди крайнее состояние S_n заметно влияет на итоговые метрики.
...
На опорных точках получаем: при n = 8 имеем M_зан = 3.36 и K_загр = 0.42; при n = 9 — 3.392 и 0.377.

На кривой видно, что после n = 8 среднее число занятых операторов растёт уже слабо: добавление новых операторов дальше создаёт прежде всего запас по отказам, а не существенный прирост фактической занятости.
...
Для сравнения: при n = 8 вероятность отказа равна 0.015, а при n = 9 — 0.006.

Кривая отказов показывает, что условие P_отк < 0.01 впервые выполняется при n = 9.

В этой точке вероятность отказа равна 0.006, поэтому значение n = 9 принимается как минимально достаточное для текущего варианта.

#### Короткий вывод

По графику отказов условие P_отк < 0.01 впервые выполняется при n = 9.
...
По расчёту: при n = 5 рост m от 1 до 5 снижает P_отк с 0.091 до 0.017, а при m = 15 вероятность отказа становится < 0.001.
...
При том же n = 5 среднее число занятых операторов увеличивается лишь с 3.102 до 3.411, а коэффициент загрузки — с 0.62 до 0.682.
...
При n = 5 переход от m = 1 к m = 15 увеличивает вероятность существования очереди с 0.091 до 0.238, а среднюю длину очереди — с 0.091 до 0.739.

При этом занятость одного места изменяется с 0.091 до 0.049; по этому показателю важно смотреть на конкретные численные значения для выбранного режима нагрузки.
...
На характерных точках видно: при m = 5 увеличение числа операторов с n = 5 до n = 8 снижает P_отк с 0.017 до < 0.001; при n = 11 оно остаётся на уровне < 0.001.
...
Для m = 5 среднее число занятых операторов меняется с 3.354 при n = 5 до 3.412 при n = 11, а коэффициент загрузки падает с 0.671 до 0.31.
...
При m = 5 вероятность существования очереди снижается с 0.211 при n = 5 до 0.011 при n = 8 и до < 0.001 при n = 11.

Средняя длина очереди за тот же переход падает с 0.481 до 0.018 и далее до < 0.001.
...
При данных параметрах a = 3.412, поэтому условие ρ_n < 1 впервые выполняется только при n = 4.

Поэтому режимы n = 1, 2, 3 остаются нестационарными и в графиках не получают выдуманных стационарных значений.
...
В первой стационарной точке n = 4 получаем P_ож = 0.695, поэтому уже первая стационарная точка соответствует высокой вероятности ожидания.
...
На первых стационарных точках M_зан остаётся равным 3.412, но коэффициент загрузки падает с 0.853 при n = 4 до 0.569 при n = 6 и 0.379 при n = 9.
...
По расчёту: при n = 4 имеем P_оч = 0.593 и L_оч = 4.031; уже при n = 6 эти значения снижаются до 0.092 и 0.213.

При n = 9 очередь практически исчезает: P_оч = 0.003, L_оч = 0.006.
...
Нестационарными остаются режимы n = 1, 2, 3; для них стационарные метрики в отчёте не подставляются.

Первое стационарное значение достигается при n = 4.
...
- λ = 0.059 1/с, μ = 0.017 1/с, ν = 1/Tw = 0.01 1/с.
...
Оставшийся вклад не превышает 5.669e-14 по вероятности и 8.092e-13 по среднему числу заявок в очереди, поэтому он пренебрежим в пределах учебной точности отчёта.
...
На характерных точках: при n = 1 получаем M_зан = 0.99 и K_загр = 0.99; при n = 5 — 3.241 и 0.648; при n = 8 — 3.404 и 0.425.
...
Для сравнения: при n = 1 вероятность существования очереди равна 0.957, а средняя длина очереди — 4.384.

При n = 5 эти значения снижаются до 0.157 и 0.308, а при n = 8 — до 0.009 и 0.015.
...
Оставшийся после усечения вклад не превышает 5.669e-14 по вероятности и 8.092e-13 по среднему числу заявок в очереди, поэтому их вклад пренебрежим в пределах учебной точности данного отчёта.

Например, при n = 1 вероятность существования очереди составляет 0.957, а средняя длина очереди равна 4.384; при n = 8 эти значения снижаются до 0.009 и 0.015.
...
- λ = 1/Tc = 0.009 1/мин, μ = 1/Ts = 0.023 1/мин.
...
На опорных точках получаем: при r = 1 имеем M_пр = 32.512 и M_ож = 31.512; при r = 10 — 11.869 и 2.574.
...
Для сравнения: P_ож убывает с 1 при r = 1 до 0.654 при r = 10 и до < 0.001 при r = 20.

В характерной точке r = 10 вероятность того, что новый отказ будет ожидать обслуживания, равна 0.654, тогда как стационарная вероятность наличия очереди составляет 0.608.
...
На характерных точках: при r = 5 имеем M_зан = 5 и K_загр = 1; при r = 10 — 9.296 и 0.93.

Даже при r = 33 в среднем заняты лишь 10.033 наладчика, поэтому коэффициент загрузки снижается до 0.304.
...
На концах диапазона r = 1 и r = 35 значения P_ож составляют 1 и 0 соответственно.

Для характерной точки r = 10 получаем P_ож = 0.654, тогда как стационарная вероятность наличия очереди равна 0.608; среднее число ожидающих станков при этом составляет 2.574.
