...
- λ = 1/Tc = 0.067 1/с, μ = 1/Ts = 0.015 1/с.
...
При этих исходных данных приведённая нагрузка равна 4.467, поэтому даже в системе без очереди крайнее состояние S_n заметно влияет на итоговые метрики.
...
На опорных точках получаем: при n = 10 имеем M_зан = 4.422 и K_загр = 0.442; при n = 11 — 4.448 и 0.404.
...
Для сравнения: при n = 10 вероятность отказа равна 0.01, а при n = 11 — 0.004.

Кривая отказов показывает, что условие P_отк < 0.01 впервые выполняется при n = 11.

В этой точке вероятность отказа равна 0.004, поэтому значение n = 11 принимается как минимально достаточное для текущего варианта.
...
По расчёту: при n = 5 рост m от 1 до 5 снижает P_отк с 0.177 до 0.073, а при m = 15 вероятность отказа становится 0.017.
...
При том же n = 5 среднее число занятых операторов увеличивается лишь с 3.678 до 4.392, а коэффициент загрузки — с 0.736 до 0.878.
...
При n = 5 переход от m = 1 к m = 15 увеличивает вероятность существования очереди с 0.177 до 0.621, а среднюю длину очереди — с 0.177 до 3.721.

При этом занятость одного места изменяется с 0.177 до 0.248; по этому показателю важно смотреть на конкретные численные значения для выбранного режима нагрузки.
...
На характерных точках видно: при m = 5 увеличение числа операторов с n = 5 до n = 8 снижает P_отк с 0.073 до 0.002; при n = 11 оно остаётся на уровне < 0.001.
...
Для m = 5 среднее число занятых операторов меняется с 4.14 при n = 5 до 4.466 при n = 11, а коэффициент загрузки падает с 0.828 до 0.406.
...
При m = 5 вероятность существования очереди снижается с 0.464 при n = 5 до 0.053 при n = 8 и до 0.003 при n = 11.

Средняя длина очереди за тот же переход падает с 1.289 до 0.105 и далее до 0.004.
...
При данных параметрах a = 4.467, поэтому условие ρ_n < 1 впервые выполняется только при n = 5.
...
В первой стационарной точке n = 5 получаем P_ож = 0.748, поэтому уже первая стационарная точка соответствует высокой вероятности ожидания.
...
На первых стационарных точках M_зан остаётся равным 4.467, но коэффициент загрузки падает с 0.893 при n = 5 до 0.638 при n = 7 и 0.447 при n = 10.
...
По расчёту: при n = 5 имеем P_оч = 0.668 и L_оч = 6.262; уже при n = 7 эти значения снижаются до 0.135 и 0.372.

При n = 10 очередь практически исчезает: P_оч = 0.008, L_оч = 0.015.
...
- λ = 0.067 1/с, μ = 0.015 1/с, ν = 1/Tw = 0.01 1/с.
...
Оставшийся вклад не превышает 5.369e-14 по вероятности и 9.464e-13 по среднему числу заявок в очереди, поэтому он пренебрежим в пределах учебной точности отчёта.
...
На характерных точках: при n = 1 получаем M_зан = 0.996 и K_загр = 0.996; при n = 5 — 3.936 и 0.787; при n = 8 — 4.416 и 0.552.
...
Для сравнения: при n = 1 вероятность существования очереди равна 0.981, а средняя длина очереди — 5.231.

При n = 5 эти значения снижаются до 0.337 и 0.8, а при n = 8 — до 0.043 и 0.076.
...
Оставшийся после усечения вклад не превышает 5.369e-14 по вероятности и 9.464e-13 по среднему числу заявок в очереди, поэтому их вклад пренебрежим в пределах учебной точности данного отчёта.

Например, при n = 1 вероятность существования очереди составляет 0.981, а средняя длина очереди равна 5.231; при n = 8 эти значения снижаются до 0.043 и 0.076.
...
- λ = 1/Tc = 0.01 1/мин, μ = 1/Ts = 0.019 1/мин.
...
На опорных точках получаем: при r = 1 имеем M_пр = 28.981 и M_ож = 27.981; при r = 10 — 12.023 и 2.626.
...
Для сравнения: P_ож убывает с 1 при r = 1 до 0.679 при r = 10 и до < 0.001 при r = 20.

В характерной точке r = 10 вероятность того, что новый отказ будет ожидать обслуживания, равна 0.679, тогда как стационарная вероятность наличия очереди составляет 0.638.
...
На характерных точках: при r = 5 имеем M_зан = 5 и K_загр = 1; при r = 10 — 9.398 и 0.94.

Даже при r = 31 в среднем заняты лишь 10.268 наладчика, поэтому коэффициент загрузки снижается до 0.331.
...
На концах диапазона r = 1 и r = 31 значения P_ож составляют 1 и 0 соответственно.

Для характерной точки r = 10 получаем P_ож = 0.679, тогда как стационарная вероятность наличия очереди равна 0.638; среднее число ожидающих станков при этом составляет 2.626.
