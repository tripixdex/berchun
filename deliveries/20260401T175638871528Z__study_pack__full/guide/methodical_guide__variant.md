...
- λ = 1/Tc = 0.05 1/с, μ = 1/Ts = 0.017 1/с.
...
При этих исходных данных приведённая нагрузка равна 2.95, поэтому даже в системе без очереди крайнее состояние S_n заметно влияет на итоговые метрики.
...
На опорных точках получаем: при n = 7 имеем M_зан = 2.89 и K_загр = 0.413; при n = 8 — 2.928 и 0.366.

На кривой видно, что после n = 7 среднее число занятых операторов растёт уже слабо: добавление новых операторов дальше создаёт прежде всего запас по отказам, а не существенный прирост фактической занятости.
...
Для сравнения: при n = 7 вероятность отказа равна 0.02, а при n = 8 — 0.007.

Кривая отказов показывает, что условие P_отк < 0.01 впервые выполняется при n = 8.
...
По графику отказов условие P_отк < 0.01 впервые выполняется при n = 8.
...
По расчёту: при n = 5 рост m от 1 до 5 снижает P_отк с 0.059 до 0.007, а при m = 15 вероятность отказа становится < 0.001.
...
При этом занятость одного места изменяется с 0.059 до 0.021; по этому показателю важно смотреть на конкретные численные значения для выбранного режима нагрузки.
...
На характерных точках видно: при m = 5 увеличение числа операторов с n = 5 до n = 8 снижает P_отк с 0.007 до < 0.001; при n = 11 оно остаётся на уровне < 0.001.
...
При данных параметрах a = 2.95, поэтому условие ρ_n < 1 впервые выполняется только при n = 3.
...
В первой стационарной точке n = 3 получаем P_ож = 0.969, поэтому уже первая стационарная точка соответствует высокой вероятности ожидания.
...
На первых стационарных точках M_зан остаётся равным 2.95, но коэффициент загрузки падает с 0.983 при n = 3 до 0.59 при n = 5 и 0.369 при n = 8.
...
По расчёту: при n = 3 имеем P_оч = 0.953 и L_оч = 57.152; уже при n = 5 эти значения снижаются до 0.132 и 0.322.

При n = 8 очередь практически исчезает: P_оч = 0.004, L_оч = 0.007.
...
- λ = 0.05 1/с, μ = 0.017 1/с, ν = 1/Tw = 0.01 1/с.
...
На характерных точках: при n = 1 получаем M_зан = 0.978 и K_загр = 0.978; при n = 5 — 2.857 и 0.571; при n = 8 — 2.947 и 0.368.
...
Для сравнения: при n = 1 вероятность существования очереди равна 0.915, а средняя длина очереди — 3.442.
...
На опорных точках получаем: при r = 1 имеем M_пр = 30.5 и M_ож = 29.5; при r = 10 — 10.63 и 1.681.
...
Для сравнения: P_ож убывает с 1 при r = 1 до 0.532 при r = 10 и до < 0.001 при r = 20.

В характерной точке r = 10 вероятность того, что новый отказ будет ожидать обслуживания, равна 0.532, тогда как стационарная вероятность наличия очереди составляет 0.476.
...
На характерных точках: при r = 5 имеем M_зан = 5 и K_загр = 1; при r = 10 — 8.948 и 0.895.

Даже при r = 33 в среднем заняты лишь 9.429 наладчика, поэтому коэффициент загрузки снижается до 0.286.
...
На концах диапазона r = 1 и r = 33 значения P_ож составляют 1 и 0 соответственно.

Для характерной точки r = 10 получаем P_ож = 0.532, тогда как стационарная вероятность наличия очереди равна 0.476; среднее число ожидающих станков при этом составляет 1.681.
